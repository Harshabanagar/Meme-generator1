...
import { generateAIMemes, getPopularTemplates, generateGeminiMemeText } from './ai-meme-generator.js';
const useGeminiCaptionCheckbox = document.getElementById('use-gemini-caption');
...
const handleAIModeClick = () => {
  ...
  // Show popular templates if no results are displayed
  if (aiResultsContainer.children.length === 0) {
    displayPopularTemplates();
  }
};

// AI Meme Generation Functions
const handleGenerateAIMeme = async () => {
  const prompt = aiPromptInput.value.trim();
  if (!prompt) {
    toastAlert('Please enter a prompt to generate AI memes', 'warning');
    return;
  }
  generateAIBtn.textContent = 'Generating...';
  generateAIBtn.disabled = true;
  aiResultsContainer.innerHTML = '<div class="ai-loading">Generating AI memes...</div>';
  try {
    let memeData;
    if (useGeminiCaptionCheckbox && useGeminiCaptionCheckbox.checked) {
      // Use Gemini for captions
      const geminiCaption = await generateGeminiMemeText(prompt);
      // Use a random template for demo, or you can match template as before
      const templates = getPopularTemplates(1);
      memeData = templates.map(template => ({
        ...template,
        texts: [geminiCaption]
      }));
    } else {
      memeData = await generateAIMemes(prompt);
    }
    displayAIMemeResults(memeData);
  } catch (error) {
    console.error('AI meme generation failed:', error);
    toastAlert('Failed to generate memes. Please try again.', 'danger');
    aiResultsContainer.innerHTML = '';
  } finally {
    generateAIBtn.textContent = 'Generate AI Meme';
    generateAIBtn.disabled = false;
  }
};

const displayPopularTemplates = () => {
  const popularTemplates = getPopularTemplates(4);
  displayAIMemeResults(popularTemplates);
};

const displayAIMemeResults = (memes) => {
  aiResultsContainer.innerHTML = '';
  if (memes.length === 0) {
    aiResultsContainer.innerHTML = '<p class="text-muted text-center">No memes found. Try a different prompt.</p>';
    return;
  }
  memes.forEach(meme => {
    const memeElement = document.createElement('div');
    memeElement.className = 'ai-meme-result';
    memeElement.innerHTML = `
      <img src="${meme.image}" alt="${meme.name}" loading="lazy">
      <p>${meme.name}</p>
      <div class="ai-meme-caption">${Array.isArray(meme.texts) ? meme.texts.join('<br>') : meme.texts}</div>
    `;
    memeElement.addEventListener('click', () => {
      loadMemeIntoEditor(meme);
    });
    aiResultsContainer.appendChild(memeElement);
  });
};

const loadMemeIntoEditor = (meme) => {
  // Load the image
  const image = new Image();
  image.addEventListener('load', () => {
    ...
    // Add AI-generated texts
    if (meme.texts && meme.texts.length > 0) {
      meme.texts.forEach((text, index) => {
        if (text && text.trim()) {
          const textboxData = {
            text: text,
            offsetY: index === 0 ? -180 : 180, // Top and bottom positioning
            fontSize: 40,
            fontWeight: 'bold',
            fillColor: '#ffffff',
            strokeColor: '#000000',
    ...
    toastAlert(`Loaded "${meme.name}" template with AI-generated text!`, 'success');
  });
  
  image.src = meme.image;
};

// Handle Enter key in AI prompt input
const handleAIPromptKeyDown = (evt) => {
  if (evt.key === 'Enter') {
    evt.preventDefault();
...
// AI event listeners
manualModeBtn.addEventListener('click', handleManualModeClick);
aiModeBtn.addEventListener('click', handleAIModeClick);
generateAIBtn.addEventListener('click', handleGenerateAIMeme);
aiPromptInput.addEventListener('keydown', handleAIPromptKeyDown);
